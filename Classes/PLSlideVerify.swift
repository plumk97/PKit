...
open class PLSlideVerify: UIView {
    public typealias SucceedCallback = ()->Void
    
    open class Config: NSObject {
        open var data: Any? // url/image
        open var xR: CGFloat = 0.5 // 0 - 1
        open var yR: CGFloat = 0.5 // 0 - 1
        
        public init(data: Any?, xR: CGFloat = 0.5, yR: CGFloat = 0.5) {
            super.init()
            self.data = data
            self.xR = xR
            self.yR = yR
    ...
    open func show(succeed: @escaping SucceedCallback) {
