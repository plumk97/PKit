...
class PLMediaBrowser: UIViewController {
    ...
    /// 翻页Callback
    typealias DidChangePageCallback = (PLMediaBrowser, Int)->Void
    ...
    /// 当前数据源
    private(set) var mediaArray = [PLMedia]() {
        didSet {
            self.updatePageTips()
        }
    }
...
class PLMediaBrowserAnimatedTransitioning: NSObject, UIViewControllerAnimatedTransitioning {
    var isPresent = false
