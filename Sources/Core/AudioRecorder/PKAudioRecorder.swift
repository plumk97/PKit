...
open class PKAudioRecorder: NSObject, AVAudioRecorderDelegate {
    ...
    public static let shared = PKAudioRecorder()
    
    private let queue = DispatchQueue(label: "PKAudioRecorder")
...
extension PKAudioRecorder {
    open class RecordModel: NSObject {
        ...
        open func start() {
            PKAudioRecorder.shared.start(self)
        }
        ...
        open func stop(isClear: Bool = false) {
            ...
            PKAudioRecorder.shared.stop(self)
        }
