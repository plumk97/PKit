...
@objc public protocol PLTabBarDelegate: NSObjectProtocol {
...
open class PLTabBar: UIView {
    
    open weak var delegate: PLTabBarDelegate?
    
    /// 内容高度 去掉safe边距
    open var contentHeight: CGFloat = 0 { didSet { self.setNeedsLayout() }}
    
    ///
    open var items: [PLTabBarItem]? { didSet { self.reloadItems(oldValue) }}
    
    open private(set) var selectedIndex = 0
    
    /// 当前选中item
    open var selectedItem: PLTabBarItem? {
