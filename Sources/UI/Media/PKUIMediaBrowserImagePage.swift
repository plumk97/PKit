...
open class PKUIMediaBrowserImagePage: PKUIMediaBrowserPage {

    open override var coverImage: UIImage? {
    ...
    open func parseData(data: PKUIMediaData, complete: ((UIImage?)->Void)?) {
    ...
    open func parseURL(_ url: URL, complete: ((UIImage?)->Void)?) {
        if url.isFileURL {
    ...
    open func update(isReset: Bool = true) {
        defer {
