...
open class PKUIPageScrollView: UIScrollView {
    
    open weak var pldelegate: UIScrollViewDelegate?
    
    open private(set) var headerView: UIView?
    open private(set) var scrollViews = [UIScrollView]()
    
    /// 当前显示Index
    open private(set) var pageIndex: Int = 0
    
    /// 分页ScrollView
    open private(set) var pageScrollView: UIScrollView!
    ...
    open func setHeaderView(_ headerView: UIView) {
    ...
    open func setScrollViews(_ scrollViews: [UIScrollView]) {
...
fileprivate var kPKUIPageScrollView = "kPKUIPageScrollView"
...
extension UIScrollView {
    ...
    static func exchange() {
        if isExchanged == false {
            ...
            let m2 = class_getInstanceMethod(self, #selector(getter: self.PKUIPageScrollView_isDragging))
            method_exchangeImplementations(m1!, m2!)
            isExchanged = true
        }
    }
    
    @objc fileprivate var PKUIPageScrollView_isDragging: Bool {
        if self is PKUIPageScrollView {
            return self.PKUIPageScrollView_isDragging
        }
        return self.pl_pageScrollView?.isDragging ?? self.PKUIPageScrollView_isDragging
    }
    
    public var pl_pageScrollView: PKUIPageScrollView? {
        return self.superview?.superview as? PKUIPageScrollView
    }
}
