...
open class PKUIRippleAnimate: NSObject {
    public enum Style {
        case fill
        case stroke
    }
...
fileprivate var kPKUIRippleAnimate = "PKUIRippleAnimate"
extension PK where Base: UIView {
    
    public var rippleAnimate: PKUIRippleAnimate {
        
        var obj = objc_getAssociatedObject(self.base, &kPKUIRippleAnimate) as? PKUIRippleAnimate
        if obj == nil {
            obj = PKUIRippleAnimate.init(view: self.base)
            objc_setAssociatedObject(self.base, &kPKUIRippleAnimate, obj, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        return obj!
    }
}
