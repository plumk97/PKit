...
class PKHTTPHandler: ChannelInboundHandler, RemovableChannelHandler {
    ...
    /// 当前请求环境
    var ctx: PKHTTPContext?
    
    /// 当前请求接口回调
    var callback: PKWebServer.HTTPHandleCallback?
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        case let .head(head):
            ...
            let ctx = PKHTTPContext(ctx: context, head: head)
            self.ctx = ctx
    ...
    /// 处理静态文件
    /// - Parameter path:
    func handleStaticFiles(path: String, ctx: PKHTTPContext) -> Bool {
