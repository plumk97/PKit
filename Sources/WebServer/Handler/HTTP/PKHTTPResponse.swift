...
public class PKHTTPResponse {
    ...
    /// 响应HTML
    /// - Parameter filepath:
    public func responseHTML(filepath: String) throws {
    ...
    public func responseHTML(html: String) {
        self.status = .ok
        self.headers["Content-Type"] = "\(PKMIMEType["html"]); charset=utf-8"
        self.body = html.data(using: .utf8)
        self.response()
    }
    ...
    public func responseStaticFile(_ filepath: String) {
        ...
        if filepath.hasSuffix(".html") || filepath.hasSuffix(".htm") {
            try? self.responseHTML(filepath: filepath)
            return
        }
